...
import type { AdSlotSpec } from '@/lib/ads';
 ...
 *
 * Na prática, isso vira duas guardas no JSX (e não uma linha de documentação
 * que alguém obedece por lembrança):
 *
 *  - o leaderboard só existe DENTRO da seção "Em alta agora". Se o ranking não
 *    tiver itens, a seção some — e o slot some com ela, em vez de escorregar
 *    para debaixo do hero;
 *  - o retângulo do fim da grade exige uma grade com corpo (`MIN_CARDS`). Num
 *    dia de 2 cards, o "fim da grade" fica a um palmo do hero.
 *
 * TODO: trocar estes dois literais pelos helpers de `lib/ads.ts` quando a
 * função da home entrar (outro agente). O contrato (`AdSlotSpec`) é o mesmo, e
 * `<AdSlot>` não renderiza nada sem Publisher ID — em desenvolvimento e em
 * qualquer ambiente sem AdSense configurado, estes dois blocos são invisíveis.
 */
const HOME_LEADERBOARD: AdSlotSpec = {
  id: 'home-leaderboard',
  format: 'leader',
  placement: 'feed',
};
const HOME_GRID_END: AdSlotSpec = { id: 'home-fim-grade', format: 'rect', placement: 'feed' };
      ...
      <div className="container">
        ...
        {home.trending.length > 0 && (
          <section className="section" aria-labelledby="em-alta-titulo">
            ...
            */}
            ...
            {/* Leaderboard DENTRO da seção do ranking: sem ranking, sem slot —
                é o que impede o anúncio de escorregar para debaixo do hero. */}
            <AdSlot slot={HOME_LEADERBOARD} />
          </section>
        )}
        ...
        {home.feed.length > 0 && (
          <section className="section" aria-labelledby="repercussao-titulo">
            ...
            {home.feed.length >= GRID_END_AD_MIN_CARDS && <AdSlot slot={HOME_GRID_END} />}
          </section>
        )}
