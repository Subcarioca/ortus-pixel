...
/**
 * =============================================================================
 * ENQUADRAMENTO DA IMAGEM — recorte automático, imagem completa ou ponto focal
 * =============================================================================
 *
 * Compartilhado entre `article-create-form.tsx`/`article-edit-form.tsx` (a
 * capa da matéria) e `block-editor.tsx` (cada imagem do corpo do texto) — é
 * literalmente o mesmo controle, com o mesmo `imageUrl`+`value`+`onChange`,
 * usado em dois contextos que sofrem do mesmo corte automático de `.thumb`.
 *
 * -----------------------------------------------------------------------------
 * POR QUE O SELETOR DE PONTO FOCAL, E NÃO UM RECORTE DE PIXELS DE VERDADE
 * -----------------------------------------------------------------------------
 * Ver o cabeçalho de `@subcarioca/core` (`cover-image.ts`) para a decisão
 * completa. Em resumo: um recorte de pixels de verdade (arrastar um
 * retângulo, gravar uma SEGUNDA imagem já cortada) exigiria processamento de
 * imagem no servidor — escopo bem maior do que "escolher o enquadramento". O
 * ponto focal resolve o mesmo problema prático (preservar o que importa na
 * foto) só com CSS (`object-position`), sem nenhuma biblioteca nova.
 *
 * -----------------------------------------------------------------------------
...
import { COVER_IMAGE_FITS, COVER_IMAGE_FIT_LABELS, type CoverImageFit } from '@subcarioca/core';
...
interface CoverImageFitFieldProps {
  /** URL da imagem já digitada/enviada no campo acima — a mesma que vai ser gravada. */
  imageUrl: string;
  value: CoverImageFitValue;
  onChange: (next: CoverImageFitValue) => void;
  /** Nome do `role="radiogroup"` — precisa ser único quando há mais de um
   *  campo destes na mesma tela (ex.: vários blocos de imagem no editor). */
  groupName: string;
  className?: string;
}
...
export function CoverImageFitField({
  ...
  return (
    <div className={className}>
      <span className="form-hint">Enquadramento</span>

      <div className="admin-actions" role="radiogroup" aria-label="Enquadramento da imagem">
            ...
            <input
              ...
              onChange={() =>
                onChange({
                  ...
                  // `parseCoverImageFocus` em `@subcarioca/core`), mas é
                  // melhor a tela já refletir isso do que depender da
        ...
        (imageUrl ? (
          <>
            ...
            <span className="form-hint">
              ...
              {value.focalX === null && ' Nenhum ponto marcado ainda; não salva sem um.'}
            </span>
          </>
        ) : (
          <span className="form-hint">Adicione uma imagem acima para marcar o ponto.</span>
        ))}
    </div>
  );
}
